...
const LoginForm = () => {
  ...
  const onOverlayClick = (e: React.MouseEvent) => {
    if (!targetContainer.current?.contains(e.target as Node)) setShowLoginForm(false);
  };
  ...
  return (
    <>
      ...
      {/* <!-- Modal toggle --> */}
      <button onClick={() => setShowLoginForm(true)} className="block text-base bg-green hover:bg-dark rounded-lg px-4 py-3 self-center">
        Login
      </button>
      ...
      {showLoginForm ? (
        <>
          {/* <!-- Main modal --> */}
          <div onClick={isOnClickOutsideTargetContainer ? onOverlayClick : undefined} className="flex fixed z-50 w-full md:inset-0 justify-center items-center">
            <div className="relative p-4 w-full max-w-md h-auto">
              {/* <!-- Modal content --> */}
              <div className="relative bg-white rounded-lg shadow" ref={targetContainer}>
                <button onClick={() => setShowLoginForm(false)} className="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-light hover:text-dark rounded-lg text-sm p-1.5 inline-flex items-center ">
          ...
          <div className='!ml-0 opacity-25 inset-0 fixed w-full z-40 bg-black'></div>
        </>
      ) : null}
    </>
  )
